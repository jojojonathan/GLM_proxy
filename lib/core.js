...
export function loadConfig({ format = "openai", defaultPort = DEFAULT_PORTS[format] } = {}) {
  ...
  // Client identity headers cache TTL — the runtime file is re-read at most this often
  const TOKEN_TTL_MS  = 5 * 60 * 1000;
  // Token file background stat-poll interval. inotify is unreliable across the
  // bind mount (WSL /mnt/c is 9p), so freshness comes from this timer. Disk is
  // touched only at startup, on this timer, and on the first request after a
  // 401 invalidation — the request path itself never reads the file.
  const TOKEN_WATCH_INTERVAL_MS = (parseInt(process.env.TOKEN_WATCH_INTERVAL_SEC || "30", 10) || 30) * 1000;
  ...
  return {
    ...
    UPSTREAM_HOST, UPSTREAM_BASE, MODEL_CONFIG_PATH, TOKEN_FILE, TOKEN_TTL_MS, TOKEN_WATCH_INTERVAL_MS,
...
export function createTokenLayer(config, log) {
  ...
  // Return the cached token. Disk is read only when the cache is empty — at
  // startup, or after invalidateToken() on a 401. Freshness is the background
  // watcher's job (startWatch); requests never touch the disk.
  function getToken() {
    if (!_token) {
      _token = loadToken();
  ...
  function startWatch() {
    fs.watchFile(config.TOKEN_FILE, { interval: config.TOKEN_WATCH_INTERVAL_MS }, () => {
      try {
        _token = loadToken();
...
// The cloud runs an agent-identity filter (deployed 2026-09-22): the phrase
// "you are <zcode|claude code|codex>" — case-insensitive, any position, in any
// message — makes the model backend answer 406 "Upstream error" (empty body;
// distinct from the banner gate's 400). Everything else passes: "You are
// Cursor", "You are an interactive ZCode agent", etc. Rewrite the flagged
// phrasing to a neutral one so coding-agent clients keep working. If the
// upstream widens its list, this regex is the place to extend.
const IDENTITY_FILTER_RE = /\b(you\s+are)\s+(?:an?\s+)?(?:zcode|claude\s*code|codex)\b/gi;
// 2026-09-24 词表扩大：子代理句式 "an agent for ZCode CLI" 也触发 406（重放二分定案；
// "You are a ZCode agent" 反而放行，过滤的是 "agent for <client>" 结构而非客户端名本身）
const AGENT_FOR_RE = /\bagent\s+for\s+(?:zcode|claude\s*code|codex)\b/gi;
function scrubIdentityPhrases(text) {
  if (typeof text !== "string") return text;
  return text
    .replace(IDENTITY_FILTER_RE, "$1 the agent")
    .replace(AGENT_FOR_RE, "agent for the CLI");
}

function scrubMessages(messages) {
  return messages.map((m) => {
    if (typeof m?.content === "string") return { ...m, content: scrubIdentityPhrases(m.content) };
    if (Array.isArray(m?.content)) {
      return { ...m, content: m.content.map((p) =>
        typeof p === "string" ? scrubIdentityPhrases(p)
          : p?.type === "text" && typeof p?.text === "string" ? { ...p, text: scrubIdentityPhrases(p.text) } : p) };
    }
    return m;
  });
}
...
function buildSanitizedBody(openAIBody, upstreamModelId) {
  const sanitized = {
    ...
    messages: scrubMessages(injectSystemBanner(openAIBody.messages || [])),
    stream: true,
  };
